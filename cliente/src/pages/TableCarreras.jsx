    ...
    <table className="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
      ...
      <tbody className="divide-y divide-gray-200">
        ...
        currentItems.map((materia) => (
          <tr key={materia.id}>
            <td className="whitespace-nowrap pr-1 pl-4 py-2 text-xs font-medium text-gray-900">{materia.data.semestre}</td>
            <td className="td-asig whitespace-nowrap px-2 py-2 text-xs text-gray-700">{materia.data.asignatura}</td>
            <td className="whitespace-nowrap px-2 py-2 text-xs text-gray-700">{materia.data.codigo}</td>
            <td className=" whitespace-nowrap px-2 py-2 text-xs text-gray-700">{materia.data.uc}</td>
            <td className=" td-des px-2 py-2 text-xs text-gray-700">{materia.data.descripcion}</td>
            <td className="td-des px-2 py-2 text-xs text-gray-700">{materia.data.profesores.profesorSeccion1}{materia.data.profesores.profesorSeccion2 ? ', ' + materia.data.profesores.profesorSeccion2 : ''}{materia.data.profesores.profesorSeccion3 ? ', ' + materia.data.profesores.profesorSeccion3 : ''}{materia.data.profesores.profesorSeccion4 ? ', ' + materia.data.profesores.profesorSeccion4 : ''}</td>
            <td className="td-des  text-ellipsis px-2 py-2 text-xs text-gray-700">{materia.data.idProfesores.idProfesorSeccion1}{materia.data.idProfesores.idProfesorSeccion2 ? ', ' + materia.data.idProfesores.idProfesorSeccion2 : ''}{materia.data.idProfesores.idProfesorSeccion3 ? ', ' + materia.data.idProfesores.idProfesorSeccion3 : ''}{materia.data.idProfesores.idProfesorSeccion4 ? ', ' + materia.data.idProfesores.idProfesorSeccion4 : ''}</td>
            <td className="whitespace-nowrap px-2 py-2 text-xs text-gray-700">{materia.data.prelacion}</td>
            <button
              onClick={() => handleEditar(materia)}
                  type="submit"
                  className="px-2 bg-gradient-to-r from-yellow-400 to-yellow-600 text-white py-1 rounded-lg block focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500 my-2"
              >
                  Editar
              </button>
          </tr>
